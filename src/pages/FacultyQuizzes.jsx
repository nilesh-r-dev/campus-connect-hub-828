...
import { ArrowLeft, ClipboardList, Plus, PlusCircle, MinusCircle } from "lucide-react";
            ...
            <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-4 border-t pt-4">
                  ...
                  {questions.map((q, qIndex) => (<Card key={qIndex}>
                      <CardContent className="pt-6 space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {q.options.map((opt, oIndex) => (<Input key={oIndex} value={opt} onChange={(e) => updateOption(qIndex, oIndex, e.target.value)} placeholder={`Option ${oIndex + 1}`}/>))}
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <Label>Correct Answer</Label>
                            <Select value={String(q.correctAnswer)} onValueChange={(v) => updateQuestion(qIndex, "correctAnswer", parseInt(v))}>
                              <SelectTrigger>
                                <SelectValue />
                              </SelectTrigger>
                              <SelectContent>
                                {q.options.map((_, i) => (<SelectItem key={i} value={String(i)}>Option {i + 1}</SelectItem>))}
                              </SelectContent>
                            </Select>
                          </div>
                          <div>
                            <Label>Points</Label>
                            <Input type="number" value={q.points} onChange={(e) => updateQuestion(qIndex, "points", parseInt(e.target.value))} min="1"/>
                          </div>
                        </div>
                      </CardContent>
                    </Card>))}
