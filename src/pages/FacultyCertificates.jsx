...
const FacultyCertificates = () => {
    ...
    const loadStudents = async () => {
        ...
        const studentsWithProgress = await Promise.all((enrollmentsData || []).map(async (enrollment) => {
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-primary/5 via-background to-secondary/5">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {selectedSubject && students.length === 0 ? (<div className="text-center py-16">
          ...
          </div>) : selectedSubject ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {students.map((student) => (<Card key={student.studentId} className="hover:shadow-lg transition-all">
                <CardHeader>
                ...
                <CardContent>
                  {student.hasCertificate ? (<div className="flex items-center gap-2 text-green-600">
                      <CheckCircle className="h-4 w-4"/>
                      <span className="text-sm font-medium">Certificate Issued</span>
                    </div>) : student.isComplete ? (<Button onClick={() => handleIssueCertificate(student.studentId)} className="w-full">
                      Issue Certificate
                    </Button>) : (<p className="text-sm text-muted-foreground">Complete all videos to issue certificate</p>)}
                </CardContent>
              </Card>))}
