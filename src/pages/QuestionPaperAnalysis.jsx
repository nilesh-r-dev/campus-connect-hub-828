...
const QuestionPaperAnalysis = () => {
    ...
    const getCombinedContent = () => {
        const fileParts = uploadedFiles.map((f, i) => `--- Question Paper ${i + 1}: ${f.name} ---\n${f.content}`);
    ...
    return (<div className="container mx-auto p-6 space-y-6 animate-fadeIn">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              <label htmlFor="file-upload" className="flex items-center justify-center gap-2 border-2 border-dashed border-muted-foreground/30 rounded-lg p-6 cursor-pointer hover:border-primary/50 hover:bg-muted/50 transition-colors">
                <Plus className="h-5 w-5 text-muted-foreground"/>
                <span className="text-muted-foreground text-sm">Click to add question paper files (.txt, .doc, .docx)</span>
              </label>
              <Input id="file-upload" type="file" accept=".txt,.doc,.docx,text/*" onChange={handleFileUpload} className="hidden" multiple/>
            </div>

            {uploadedFiles.length > 0 && (<div className="space-y-2">
                <p className="text-sm font-medium">Uploaded Files ({uploadedFiles.length})</p>
                <div className="flex flex-wrap gap-2">
                  {uploadedFiles.map((f) => (<Badge key={f.name} variant="secondary" className="flex items-center gap-1 pr-1">
                      <FileText className="h-3 w-3"/>
                      <span className="max-w-[150px] truncate">{f.name}</span>
                      <button onClick={() => removeFile(f.name)} className="ml-1 rounded-full hover:bg-destructive/20 p-0.5">
                        <X className="h-3 w-3"/>
                      </button>
                    </Badge>))}
                </div>
            ...
            <div className="relative">
              <Textarea placeholder="Or paste your question paper content here..." value={content} onChange={(e) => setContent(e.target.value)} className="min-h-[200px] font-mono text-sm"/>
            </div>

            <Button onClick={handleAnalyze} disabled={isAnalyzing || !hasContent} className="w-full">
              {isAnalyzing ? (<>
        ...
        <Card>
          ...
          <CardContent>
            {isAnalyzing ? (<div className="flex items-center justify-center min-h-[300px]">
                <div className="text-center space-y-3">
                  <Loader2 className="h-8 w-8 animate-spin mx-auto text-primary"/>
                  <p className="text-muted-foreground">Analyzing your question papers...</p>
                </div>
              </div>) : analysis ? (<div className="prose prose-sm dark:prose-invert max-w-none">
                <div className="whitespace-pre-wrap bg-muted p-4 rounded-lg max-h-[500px] overflow-y-auto">
                  {analysis}
                </div>
              </div>) : (<div className="flex items-center justify-center min-h-[300px] text-muted-foreground">
