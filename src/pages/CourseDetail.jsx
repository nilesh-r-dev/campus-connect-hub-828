      ...
      <main className="container mx-auto px-4 py-8">
        <Tabs defaultValue="assignments" className="w-full">
          ...
          <TabsContent value="assignments" className="mt-6">
            {assignments.length === 0 ? (<div className="text-center py-16">
                ...
                {assignments.map((assignment) => (<Card key={assignment.id} className="hover:shadow-lg transition-all">
                    <CardHeader>
                    ...
                    <CardContent className="space-y-3">
                      ...
                      {assignment.due_date && (<div className="flex items-center text-sm">
                      ...
                      <Button className="w-full" onClick={() => navigate(`/assignment/${assignment.id}`)}>
                        View & Submit
                      </Button>
                    </CardContent>
                  </Card>))}
          ...
          <TabsContent value="quizzes" className="mt-6">
            {quizzes.length === 0 ? (<div className="text-center py-16">
                <Award className="h-16 w-16 mx-auto text-muted-foreground mb-4"/>
                <h2 className="text-xl font-semibold mb-2">No Quizzes</h2>
                <p className="text-muted-foreground">No quizzes available yet.</p>
              </div>) : (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {quizzes.map((quiz) => (<Card key={quiz.id} className="hover:shadow-lg transition-all">
                    <CardHeader>
          ...
          <TabsContent value="notes" className="mt-6">
            {notes.length === 0 ? (<div className="text-center py-16">
                ...
                {notes.map((note) => (<Card key={note.id} className="hover:shadow-lg transition-all">
                    <CardHeader>
                    ...
                    <CardContent className="space-y-3">
                      ...
                      {note.file_url && (<Button className="w-full" variant="outline" asChild>
          ...
          <TabsContent value="videos" className="mt-6">
            {videos.length === 0 ? (<div className="text-center py-16">
                ...
                {videos.map((video) => (<Card key={video.id} className="hover:shadow-lg transition-all">
                    <CardHeader>
                      <div className="bg-accent/10 rounded-full p-3 w-fit mb-2">
                    ...
                    <CardContent className="space-y-3">
                      ...
                      {video.video_url && (<div className="aspect-video w-full bg-muted rounded-lg overflow-hidden">
                          <iframe src={video.video_url} className="w-full h-full" allowFullScreen/>
                        </div>)}
                    </CardContent>
                  </Card>))}
              </div>)}
