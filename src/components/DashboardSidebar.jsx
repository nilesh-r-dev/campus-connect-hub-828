...
import { GraduationCap, BookOpen, Video, FileText, MessageSquare, LogOut, ClipboardList, Award, Briefcase, Newspaper, FileQuestion, Upload, } from "lucide-react";
...
export function DashboardSidebar({ userRole, profile, onProfileClick, onLogout }) {
    ...
    return (<Sidebar collapsible="icon" className="border-r">
        ...
        <SidebarGroup>
          ...
          <SidebarGroupContent>
            ...
            return (<SidebarMenuItem key={item.title}>
                    <SidebarMenuButton onClick={() => navigate(item.url)} tooltip={item.title} className={cn("cursor-pointer", isActive && "bg-primary/10 text-primary font-medium")}>
      ...
      <SidebarFooter className="border-t p-4">
        <div className="flex flex-col gap-2">
          <Button variant="ghost" className={cn("w-full justify-start gap-3", collapsed && "justify-center px-2")} onClick={onProfileClick}>
          ...
          <Button variant="ghost" size={collapsed ? "icon" : "default"} className={cn("w-full text-destructive hover:text-destructive hover:bg-destructive/10", !collapsed && "justify-start gap-3")} onClick={onLogout}>
